...
- `install-user-skills.py` 末尾会安装/检查 GitHub CLI `gh`，尝试用 `gh skill install --from-local` 安装本地用户级 skills，随后安装 [playwright-cli](https://github.com/microsoft/playwright-cli) 与 skill，并从 ima agent-interface 安装 `ima-skill`、交互写入 `~/.config/ima/` 凭证。
