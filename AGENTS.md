...
- 本仓库是个人 Skills 集合：通用 Skill 直接放在 `skills/<skill-name>/`。
- 后续任何新增、删除、重命名 Skill 或脚本时，必须同步更新 `README.md`、`docs/repo-init.md`，并按影响范围更新 `docs/installation.md`。
...
- 脚本验证：`install-user-skills.py` 修改后运行 `python3 -m py_compile scripts/install-user-skills.py`；新增或修改 Bash 脚本后运行 `bash -n scripts/*.sh`；安装路径或参数变化时同步更新安装文档。
