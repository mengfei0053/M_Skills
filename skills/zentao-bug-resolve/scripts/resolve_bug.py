#!/usr/bin/env python3
"""Resolve a ZenTao bug via session login + bug/resolve page API.
...
class ResolveError(RuntimeError):
    """Raised when the resolve workflow cannot continue."""


def parse_args() -> argparse.Namespace:
    """Parse CLI arguments."""
    parser = argparse.ArgumentParser(
        description="Resolve a ZenTao bug and optionally add a remark.",
    )
    parser.add_argument(
        "bug",
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Remark text. Prefer --comment for multi-word text.",
    )
    parser.add_argument(
        "--comment",
        dest="comment_flag",
        default=None,
        help="Remark text attached to the resolve action.",
    )
    parser.add_argument(
        "--resolution",
        choices=RESOLUTIONS,
        default=DEFAULT_RESOLUTION,
        help=f"Resolution type. Default: {DEFAULT_RESOLUTION}.",
    )
    parser.add_argument(
        "--resolved-build",
        default=None,
        help="Resolved build ID/name. Default: form first build or trunk.",
    )
    parser.add_argument(
        "--assigned-to",
        default=None,
        help="Assignee after resolve. Default: resolve-form assignedTo/openedBy.",
    )
    parser.add_argument(
        "--duplicate-bug",
        default=None,
        help="Duplicate bug ID when --resolution=duplicate.",
    )
    parser.add_argument(
        "--auth-file",
        type=Path,
    ...
    parser.add_argument(
        "--skip-verify",
        action="store_true",
        help="Skip REST verification of resolved status/history.",
    )
    parser.add_argument(
        "--format",
        choices=("text", "json"),
...
def parse_bug_id(raw_bug: str) -> str:
    ...
    raise ResolveError(f"无法从输入中解析 bug ID: {raw_bug}")


def resolve_comment(positional: str | None, flagged: str | None) -> str:
    """Resolve remark text from positional or --comment (optional)."""
...
def validate_server_url(server: str) -> str:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise ResolveError("ZENTAO_URL 必须是 http(s) URL")
...
class ZenTaoSession:
    ...
    def request(
        ...
        *,
        ...
        request_headers = {
            "User-Agent": "zentao-bug-resolve/1.0",
            "Accept": "*/*",
        }
        if headers:
        ...
        try:
            ...
            raise ResolveError(f"请求失败: {url}: {error}") from error
    ...
    def request_json(
        ...
        *,
        ...
        try:
            ...
            raise ResolveError(f"响应不是 JSON ({status}): {url}") from error
...
def session_login(session: ZenTaoSession, auth: Mapping[str, str]) -> None:
    ...
    if not isinstance(session_data, dict) or not session_data.get("sessionID"):
        raise ResolveError("无法获取禅道 sessionID")
    ...
    if not isinstance(login_data, dict) or login_data.get("rand") is None:
        raise ResolveError("无法获取登录 rand")
    ...
    if login_result.get("status") != "success" or "user" not in login_result:
        ...
        raise ResolveError(f"禅道会话登录失败: {message}")


def account_from_value(value: Any) -> str:
    """Normalize assignedTo/openedBy values to account string."""
...
def pick_resolved_build(form: Mapping[str, Any], override: str | None) -> str:
    """Choose resolvedBuild from CLI override or resolve form."""
    if override:
        return override
    ...
    if isinstance(bug, dict):
        ...
        if isinstance(opened_build, str) and opened_build:
            return opened_build.split(",", 1)[0]
...
def fetch_resolve_form(
    session: ZenTaoSession,
    server: str,
    bug_id: str,
) -> dict[str, Any]:
    """Load bug resolve form JSON."""
    payload = session.request_json(
        f"{server}/index.php?m=bug&f=resolve&bugID={bug_id}&t=json"
    )
    data = maybe_parse_embedded_json(payload.get("data"))
    if not isinstance(data, dict) or "bug" not in data:
        title = data.get("title") if isinstance(data, dict) else None
        if title and ("登录" in str(title) or "login" in str(title).lower()):
            raise ResolveError("打开解决表单失败：会话未登录")
        raise ResolveError("打开解决表单失败：响应缺少 bug 字段")
    return data
...
def post_bug_resolve(
    ...
    *,
    ...
    assigned_to: str,
    comment: str,
    duplicate_bug: str | None,
) -> None:
    """Post resolve action through bug/resolve page API."""
    data: dict[str, Any] = {
    ...
    if resolution == "duplicate":
        if not duplicate_bug:
            raise ResolveError("resolution=duplicate 时必须提供 --duplicate-bug")
        data["duplicateBug"] = duplicate_bug

    status, body, _ = session.request(
        f"{server}/index.php?m=bug&f=resolve&bugID={bug_id}",
        method="POST",
        data=data,
        headers={
            "X-Requested-With": "XMLHttpRequest",
    ...
    if status >= 400:
        raise ResolveError(f"解决请求失败 HTTP {status}")

    success = False
    if '"result":"success"' in body or '"result": "success"' in body:
        success = True
    elif "保存成功" in body:
        success = True
    else:
    ...
    if not success:
        ...
        raise ResolveError(f"解决接口未返回成功: {snippet}")
...
def fetch_api_token(auth: Mapping[str, str], timeout: float) -> str:
    ...
    try:
        ...
        raise ResolveError(f"获取 API Token 失败: {error}") from error

    token = payload.get("token") if isinstance(payload, dict) else None
    if not token:
        raise ResolveError("登录响应中没有 token")
    return str(token)
...
def fetch_bug_detail(
    ...
    """Fetch bug detail via REST API for verification."""
    request = urllib.request.Request(
        f"{server}/api.php/v1/bugs/{bug_id}",
        headers={"Token": token, "User-Agent": "zentao-bug-resolve/1.0"},
        method="GET",
    )
    try:
        with urllib.request.urlopen(request, timeout=timeout) as response:
        ...
        raise ResolveError(f"读取 Bug 详情失败: {error}") from error

    if not isinstance(payload, dict):
        raise ResolveError("Bug 响应格式异常")
    return payload


def extract_actions(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    """Normalize bug.actions to a list of dicts."""
...
def find_resolved_action(
    actions: list[dict[str, Any]],
    comment: str,
) -> dict[str, Any] | None:
    """Find the newest matching resolved action."""
    normalized = comment.strip()
    candidates: list[dict[str, Any]] = []
    for action in actions:
        if action.get("action") != "resolved":
            continue
...
def summarize_user(value: Any) -> Any:
    """Return a compact user representation for JSON output."""
    if isinstance(value, dict):
        return {
            "id": value.get("id"),
            "account": value.get("account"),
...
def main() -> int:
    ...
    try:
        ...
        if args.resolution == "duplicate" and not args.duplicate_bug:
            raise ResolveError("resolution=duplicate 时必须提供 --duplicate-bug")

        session = ZenTaoSession(timeout=args.timeout)
        session_login(session, auth)
        form = fetch_resolve_form(session, server, bug_id)
        bug = form.get("bug") if isinstance(form.get("bug"), dict) else {}

        assigned_to = args.assigned_to or account_from_value(form.get("assignedTo"))
        if not assigned_to:
            assigned_to = account_from_value(bug.get("openedBy"))
        resolved_build = pick_resolved_build(form, args.resolved_build)
        ...
        bug_after: dict[str, Any] | None = None
        if not args.skip_verify:
            token = fetch_api_token(auth, args.timeout)
            bug_after = fetch_bug_detail(server, bug_id, token, args.timeout)
            if bug_after.get("status") != "resolved":
                raise ResolveError(
                    f"解决接口已返回成功，但 Bug 状态仍为 {bug_after.get('status')!r}"
                )
            if str(bug_after.get("resolution") or "") != args.resolution:
                raise ResolveError(
                    "解决接口已返回成功，但 resolution 与预期不一致: "
                    f"{bug_after.get('resolution')!r}"
                )
            matched = find_resolved_action(extract_actions(bug_after), comment)
            if matched is None:
                raise ResolveError(
                    "解决接口已返回成功，但未能在历史中验证到对应 resolved 记录"
                )
        ...
        result = {
            ...
            "server": server,
            "bug": None
            if bug_after is None
            else {
                "status": bug_after.get("status"),
                "resolution": bug_after.get("resolution"),
                "resolvedBy": summarize_user(bug_after.get("resolvedBy")),
                "resolvedDate": bug_after.get("resolvedDate"),
                "resolvedBuild": bug_after.get("resolvedBuild"),
            },
            "action": None
            if matched is None
            else {
        ...
        if args.format == "json":
            ...
            print(f"解决 Bug 失败: {error}", file=sys.stderr)
        return 1

    if args.format == "json":
        print(json.dumps(result, ensure_ascii=False))
    else:
        print(f"已将 Bug #{bug_id} 标记为 resolved（{args.resolution}）。")
        if comment:
            print(f"备注: {comment}")
        if matched:
            print(
