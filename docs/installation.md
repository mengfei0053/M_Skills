...
7. 安装完成后先检查 `gh auth status`；如果 GitHub CLI 已登录，跳过 `bw sync`、`bw get password github_gh_token`、写入 `~/.config/m_skill_auths/gh_token` 和 `gh auth login`。仅未登录时才通过 `BW_SESSION` 同步 Bitwarden vault，读取并写入 GitHub token 后登录 `gh`。
