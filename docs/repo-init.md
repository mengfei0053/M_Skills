...
| `scripts/install-user-skills.py` | 跨平台安装 `skills/user/` 到用户级工具配置；自动查找包含 `skills/user/` 的仓库根目录并支持 `M_SKILLS_REPO_DIR` 覆盖；在 Linux 上按 GitHub CLI 官方说明安装/检查 `gh` 并尝试用 `gh skill install --from-local` 安装本地 skills；安装 [playwright-cli](https://github.com/microsoft/playwright-cli) 与 skill；从 [ima agent-interface](https://ima.qq.com/agent-interface) 安装 `ima-skill` 并提示 `~/.config/ima/` 凭证 | 修改后运行 `python3 -m py_compile scripts/install-user-skills.py` |
